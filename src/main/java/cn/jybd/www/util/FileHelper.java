...
public class FileHelper {
	...
	public static void info(Object o) {
		...
		File f = file.get();
		if(f == null)throw new RuntimeException("日志文件没有初始化，请确保日志文件已经生成，相关方法请参考FileHelper.initLogFile(String name)");
	...
	public static void warn(Object o) {
		...
		try {
			...
			if(o != null) {
				//将日志写入文件中
				bw.write(String.format("%s [warn]:%s", fmt(), o.toString()+"\r\n"));
				//在这里会将日志信息在控制台也打印一遍
				System.out.println(String.format("%s [warn]:%s", fmt(), o.toString()+"\r\n"));
				bw.write("\r\n");
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static void error(Object o,Throwable t) {
		...
		try {
			...
			if(o != null) {
				//将日志写入文件中
				bw.write(String.format("%s [error]:%s", fmt(), o.toString()+"\r\n"+t));
				//在这里会将日志信息在控制台也打印一遍
				System.out.println(String.format("%s [error]:%s", fmt(), o.toString()+"\r\n"+t));
				bw.write("\r\n");
			}
		} catch (IOException e) {
			e.printStackTrace();
